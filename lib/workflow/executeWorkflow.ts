...
async function finalizeWorkflowExecution(
  ...
  await prisma.workflow
    ...
    .catch((err) => {
      // igonre error caused by triggering the pipeline again when an
...
async function executePhase(
  ...
  const executionEnvironment: ExecutionEnvironment<any> =
    createExecutioonEnvironment(node, environment, logCollector);
...
function createExecutioonEnvironment(
  node: AppNode,
  environment: Environment,
  logCollector: LogCollector
...
async function cleanupEnvironment(environment: Environment) {
  if (environment.browser) {
    await environment.browser
      .close()
      .catch((err) => console.error("cannot cloase browser, reason:", err));
  }
}
